...
mkdir fails && \
php -d memory_limit=-1 /usr/bin/composer install --prefer-dist
sum=$(( $sum + $? ))
...
if [[ $1 != *"victoire/victoire"* ]]; then
    ...
    php -d memory_limit=-1 /usr/bin/composer require $1:dev-$branch#$revision --prefer-dist
    sum=$(( $sum + $? ))
fi
...
nohup java -jar vendor/selenium.jar &> /dev/null &
sum=$(( $sum + $? ))
